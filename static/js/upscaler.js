...
window.UpscalerModule = (() => {
  ...
  /* 설정 상수 */
  const TILE_THRESHOLD = 512 * 512; // 262,144 px (약 512x512 초과 시 타일링)
    ...
    _tfLoadedPromise = new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        if (window.tf) {
          ...
          try {
            tf.env().set('WEBGL_FORCE_F16_TEXTURES', true);      // Float16 텍스처로 VRAM 50% 절감
  ...
  async function upscaleImage(source, scale = 2, onProgress = null) {
    ...
    const useTiling = totalPixels > TILE_THRESHOLD;

    console.log(`[Upscaler] Real-ESRGAN Compact ${scaleNum}× 시작: 원본 크기 ${srcCanvas.width}×${srcCanvas.height} (${useTiling ? '타일링 모드' : '단일 패스'})`);
  ...
  function getImageInfo(imgEl) {
    ...
    return {
      ...
      height:      h,
      totalPixels: w * h,
      willTile:    w * h > TILE_THRESHOLD,
    };
  }

  return { upscaleImage, getImageInfo, disposeModels };
